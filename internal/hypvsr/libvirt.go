...
func (h *Libvirt) Create(machine *Machine) error {
	...
	args := []string{
		...
		"--disk", fmt.Sprintf("path=%s,device=disk", fmt.Sprintf(path.GetPath(path.DiskFile), machine.Name)),
		"--disk", fmt.Sprintf("path=%s,device=disk", fmt.Sprintf(path.GetPath(path.SeedImageFile), machine.Name)),
		"--import",
