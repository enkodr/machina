...
### Results (results)
The `Result` field enables you to specify the desired results associated with the cluster. 
This can include any information or artifacts generated by the cluster during its operation. 
The results field provides a structured way to capture and store relevant data produced by the 
...
- kind: Machine
  ...
  scripts:
    install: |
      ...
      echo "Hello from server" > $(results.message)
